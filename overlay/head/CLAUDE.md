...
Recording of the web client's tRPC API is configured in `apps/web/client/appmap.yml`. The record commands and the gold traces (recorded test runs kept as a behavior baseline) are in `apps/web/client/gold_traces/manifest.yaml`, maintained with the appmap-gold-traces skill.
