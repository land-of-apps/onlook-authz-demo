...
#   - The script waits for Enter before "running" each command and before each new step.
set -uo pipefail
...
UI_PIDS=()
cleanup() { for p in "${UI_PIDS[@]:-}"; do [ -n "$p" ] && kill "$p" 2>/dev/null; done; }
trap cleanup EXIT
...
pause() {  # pause "prompt"
  [ "$AUTO" = "1" ] && { echo; return; }
  echo
  read -r -p "${DIM}  ── ${1:-Press Enter to continue} ──${OFF} " _ </dev/tty
  echo
}

step() {   # step <number> <title>
  [ "$AUTO" = "1" ] || clear
  echo "${B}${CYAN}━━━ Step $1 of 8 ━━━ $2${OFF}"
  echo
}
...
# 1 ---------------------------------------------------------------------------
step 1 "The pull request, as a reviewer saw it"
...
# 2 ---------------------------------------------------------------------------
step 2 "Record what the code does"
...
# 4 ---------------------------------------------------------------------------
step 4 "Where the fix worked: project.delete, called by a stranger"
...
# 5 ---------------------------------------------------------------------------
step 5 "Where the fix is missing: the traces with no diff"
...
# 6 ---------------------------------------------------------------------------
step 6 "Ask the recordings: which requests ran an authorization check?"
...
# 7 ---------------------------------------------------------------------------
step 7 "Browse both branches in the query UI"
...
# 8 ---------------------------------------------------------------------------
step 8 "The review this produces"
